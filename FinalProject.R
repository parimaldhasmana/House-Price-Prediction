...
# reformatting some character variables that we can easily convert to numeric
...
#  Now, time to drop off the variables that have been made numeric and are no longer needed.
...
# lets build some interactions that may be worth looking at. For example, if the house has a pool, is it more important that it has a big deck, or something like that? I used correlation visuals like below to do it
...
# Basements have been getting bigger over time, apparently. As have the sizes of the living areas
...
#partition! using the caret partitioning function.
library(caret)
...
# R-squared is not too bad!(R-squared indicates what percent of the variation in the outcome is explained using the model we designed)

houseprice.lm.pred <- predict(lm_model_15, testing)

houseprice.lm.pred 
summary(houseprice.lm.pred)
...
# That's our model with the important stuff


prediction1 <- predict(lm_model_15, testing, type="response")
model_output <- cbind(testing, prediction1)
model_output
model_output$prediction1
model_output$SalePrice

# install.packages('caret')
# library(caret)

#Test with RMSE
postResample(model_output$SalePrice, model_output$prediction)
# Not too bad, given that this is just an LM


###A Random Forest
#require(randomForest)
# Let's try training the model with an RF. Let's use all the variables and see how things look, since randomforest does its own feature selection.
model_1 <- randomForest(SalePrice ~ ., data=training)

# Predict using the test set
prediction2 <- predict(model_1, testing)
model_output <- cbind(testing, prediction2)

predicted = model_output$prediction2
actual = model_output$SalePrice
...
predicted <- model_output$prediction
actual <- model_output$SalePrice


#Test with RMSE
rmse(actual, predicted)
# Lets go ahead the xgboost
...
testing$pred_majority<-as.factor(ifelse(prediction1=='X1' & prediction2=='X1','X1',
                                        ifelse(prediction1=='X1' & prediction=='X1','X1',
                                               ifelse(prediction2=='X1' & prediction=='X1','X1','X0'))))
str(testing$SalePrice)
str(testing$pred_majority)

testing$SalePrice <- as.factor(testing$SalePrice)

ensemble.voting <- confusionMatrix(testing$SalePrice,testing$pred_majority)

levels(testing$SalePrice)
levels(testing$pred_majority)

show(testing$pred_majority)
#Ensembling using GBM 
 require(gbm)
...
testing$pred_rf_prob<-predict(object = lm_model_15,testing,type='prob')
testing$pred_lr_prob<-predict(object = model_1, testing,type='response')
#testing$pred_xb_prob<-predict(object = bstSparse, testing,type='response')
#Taking average of predictions
testing$pred_avg<-(testing$pred_rf_prob$X1+testing$pred_lr_prob$X1)/2
